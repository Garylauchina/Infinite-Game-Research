...
本文提出并实现了一个基于"市场先生"模型的交易所模拟器（Infinite Game V5.0），用于研究金融市场中的结构涌现和演化机制。

**核心设计范式一：唯一参与主体**
单一市场仅存在一个抽象的"市场先生"（Market Entity），代表着所有交易的聚合。市场先生通过多个分身（Avatar/Player）表示其参与强度，所有分身的交易行为聚合为市场的整体交易活动。

**核心设计范式二：结构密度作为奖励函数**
设计了"市场结构密度"（Market Structure Density）作为奖励函数的核心组成部分（权重0.4），驱动市场先生的参与决策。结构密度越高，体验奖励越高，市场先生越愿意增加参与强度。

**技术实现**：（1）分身采用纯随机报价，使用宏观聚合交易成功概率，不模拟微观撮合；（2）引入"混乱因子"机制，参与强度高时惩罚交易成功概率，维持系统平衡。

**实验结果**：✅ P0-P3 阶段测试已完成，所有测试均成功通过。详细结果请参考 [P0_P3_TEST_REPORT.md](P0_P3_TEST_REPORT.md)。本研究为理解金融市场的自组织机制提供了新的理论框架和实证工具。
...
**定义**：衡量市场状态轨迹在状态空间中的分布复杂程度。在本系统中，结构密度等同于"市场结构密度"，作为奖励函数的核心组成部分。
...
### 3.5 市场先生模型：唯一参与主体与结构密度奖励

**核心设计范式一：唯一参与主体**
- **单一市场，单一市场先生**：对于一个单一市场，仅存在一个抽象的"市场先生"，它是市场中所有交易的聚合代表
- **交易聚合**：所有交易活动都归属于这个单一主体，市场先生代表所有交易的统一聚合
- **分身机制**：市场先生通过多个分身（Avatar/Player）表示其参与强度，分身数量 = 参与强度
- **随机报价**：分身采用纯随机报价方式，不设计学习机制，使用宏观聚合交易成功概率
...
- **混乱因子**：当参与强度高时，混乱因子会升高，惩罚交易成功概率，维持系统平衡
- **规则提炼**：不去模拟单个交易所复杂的规则集，而是将金融市场最根本的共性规则提炼出来，简单规则也会产生复杂结构
...
**固定规则集**（实际实现）：
- 价格边界：40000 - 60000
- 固定手续费：0.05%（成交才收，无Maker/Taker区分）
...
**注意**：P0-P3 阶段测试正在进行中，实验结果将在测试完成后更新到本节。
...
2. 规则集相对简单
3. 尚未验证真实市场数据
4. 当前为 Phase 1，尚未完成系统性相图扫描

**Phase 1 下一步工作**（不修改任何底层机制）：
实验计划已归档到 `archive/TODO.md`。主要方向包括系统性相图扫描、稳态唯一性检验、结构新颖性分析等。

**Phase 2 未来工作**（引入更高层机制）：
实验计划已归档到 `archive/TODO.md`。主要方向包括规则演化、多市场交互、学习机制、信用与杠杆等。
...
1. **市场先生模型的有效性**：通过"单一市场，单一市场先生"的设计理念，我们实现了简化的市场模型，其中市场先生通过分身表示参与强度，盈亏互相抵消，结构密度作为奖励函数驱动参与决策。

2. **简单规则下的结构涌现**：通过"规则驱动 + 随机试玩"的新范式，我们设计了实验来观察简单规则下复杂结构的涌现机制。

3. **市场结构密度与参与强度的关系**：通过 P0-P3 阶段测试，我们将验证市场结构密度与市场先生参与强度（分身数量）的关系，以及结构密度反馈机制的有效性。

4. **混乱因子的平衡作用**：通过 P3 混沌因子消融测试，我们将验证混乱因子机制在维持系统平衡中的作用。

**详细实验结果**：请参考 [P0_P3_TEST_REPORT.md](P0_P3_TEST_REPORT.md) 获取完整的测试结果和分析。
...
**✅ P0-P3 阶段测试已完成**：所有测试数据已保存至 `data/phase_analysis_output/` 目录。

详细的测试规范和数据格式请参考 [PHASE_TEST_SPECIFICATION.md](PHASE_TEST_SPECIFICATION.md)。  
完整的测试结果和分析请参考 [P0_P3_TEST_REPORT.md](P0_P3_TEST_REPORT.md)。
