...
3. 结构密度作为奖励函数的核心组成部分（权重0.4），驱动市场先生的参与决策
4. 引入"混乱因子"机制维持系统平衡
...
**核心设计范式一：唯一参与主体**
- **单一市场，唯一参与主体**：一个市场中存在**唯一参与主体"市场先生"**，代表着所有交易的聚合
- **交易聚合**：所有交易活动都归属于这个单一主体，市场先生是交易聚合的统一代表
- **分身机制**：市场先生通过多个分身（RandomExperiencePlayer）表示其参与强度，分身数量 = 参与强度
- **盈亏抵消**：所有分身的盈亏互相抵消，市场先生实际支付的只是交易费用

**核心设计范式二：结构密度作为奖励函数**
- **结构密度作为奖励函数**：结构密度直接作为体验奖励的核心组成部分（权重0.4），驱动市场先生的参与决策
- **反馈循环**：结构密度高 → 体验奖励高 → 参与强度高 → 分身数量多 → 交易活跃 → 结构密度演化
...
- 所有分身都属于同一个市场先生，代表所有交易的聚合
- 分身数量代表市场先生的参与强度
- 所有分身的盈亏互相抵消，市场先生只支付交易费用
- 结构密度作为奖励函数的核心组成部分（权重0.4），直接影响体验奖励
...
### 3.5 复杂度计算器（市场结构密度）：奖励函数

**核心设计范式**：结构密度作为奖励函数的核心组成部分。

**功能**：计算市场状态轨迹的结构密度（复杂度），作为体验奖励的核心组成部分（权重0.4）。
...
**✅ P0-P3 阶段测试已完成**：所有测试均成功通过，详细结果请参考 [P0_P3_TEST_REPORT.md](P0_P3_TEST_REPORT.md)。以下研究问题已通过实验验证。
...
在当前模型中，以下现象**预期不可能内生出现**（将通过 P0-P3 测试验证）：
- 强反身性放大（自我强化失控）
- 危机态相变与结构性崩塌
- 不可逆制度失效
- 信用链断裂或能力永久丧失
...
本模拟器采用的设计范式（单一抽象主体 + 结构密度作为奖励函数）可能具有一定的普适性。未来工作的长期目标是能够持续输出"结构参数集"（包括结构密度计算方法、参与强度调整机制、反馈循环参数等），这些参数集可以作为模块化的组件植入到各种模拟实验环境中。
