...
1. **系统性相图扫描**：定位冻结相/扩张相/稳定相的参数边界
2. **稳态唯一性与多稳态检验**：判断系统是否存在多个吸引子
3. **结构新颖性与饱和时间分析**：判定结构空间是否出现饱和迹象
4. **结构–规模解耦的定量实证**：将"结构对规模不敏感"从直觉提升为统计结论

详见 [V5.0 Phase 1 研究报告](V5.0_PHASE1_REPORT.md)。
